...
async function listarAssembleias() {
  try {
    ...
    const assembleiasFiltradas = dados.filter(a => a.CondominioID === condominioID);
    ...
    assembleiasFiltradas.forEach(item => {
      ...
      const linkDocumento = item.documento
        ? `<a href="http://localhost:3000/uploads/assembleia/${item.documento}" target="_blank" class="btn-ver"><i class="fas fa-file-pdf"></i> Ver Documento</a>`
